...
// ── LANGUAGE BAR — Auto-inject on all pages ───────────────────
(function() {
  var LANGS = [
  ...
  var TRANS = {
    en:'Choose your language', fr:'Choisir la langue',
    ar:'اختر لغتك', es:'Elige tu idioma',
    tr:'Dil seç', hi:'भाषा चुनें',
    pt:'Escolher idioma', id:'Pilih bahasa'
  };

  function injectLangBar() {
    if(document.getElementById('global-lang-bar')) return;
    var saved = localStorage.getItem('lu_lang') || 'en';
    var bar = document.createElement('div');
    bar.id = 'global-lang-bar';
    bar.style.cssText = 'background:#0F0F1A;border-bottom:1px solid rgba(255,255,255,.08);padding:8px 20px;display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;font-family:DM Sans,sans-serif;';
    var label = document.createElement('span');
    label.id = 'glb-label';
    label.style.cssText = 'font-size:11px;color:rgba(255,255,255,.4);font-weight:500;';
    label.textContent = '🌍 ' + (TRANS[saved] || TRANS.en);
    var btns = document.createElement('div');
    btns.style.cssText = 'display:flex;gap:4px;flex-wrap:wrap;';
    LANGS.forEach(function(l) {
      var btn = document.createElement('button');
      btn.textContent = l.flag + ' ' + l.label;
      btn.style.cssText = 'background:transparent;border:1px solid rgba(255,255,255,.1);color:rgba(255,255,255,.5);padding:4px 10px;border-radius:20px;font-size:11px;cursor:pointer;transition:all .2s;font-family:DM Sans,sans-serif;';
      if(l.code === saved) {
        btn.style.borderColor = '#FF6B35';
        btn.style.color = '#FF6B35';
        btn.style.background = 'rgba(255,107,53,.06)';
      }
      btn.addEventListener('click', function() {
        localStorage.setItem('lu_lang', l.code);
        ...
        btn.style.color = '#FF6B35';
        btn.style.background = 'rgba(255,107,53,.06)';
        label.textContent = '🌍 ' + (TRANS[l.code] || TRANS.en);
        // Also call L() if it exists on the current page
        if(typeof L === 'function') L(l.code, null);
        if(typeof setLang === 'function') setLang(l.code, null);
        if(typeof applyLang === 'function') applyLang(l.code);
      });
      btns.appendChild(btn);
    });
    bar.appendChild(label);
    bar.appendChild(btns);
    // Insert before nav or at top of body
  ...
  if(document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', injectLangBar);
  } else {
    injectLangBar();
  }
})();
